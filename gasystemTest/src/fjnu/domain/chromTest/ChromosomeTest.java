...
public class ChromosomeTest {
	...
	public void testChromosomeIntial() {
		...
		try {
			...
			List<StringBuffer> actualEncodes = (ArrayList<StringBuffer>) field
					.get(actualChromosome);
	...
	public void testChromosomeMutateWhenPosIsStart() {
		...
		when(mockRandom.nextInt(3)).thenReturn(2);// 模拟产生的目标码是2

		actualChromosome = new Chromosome(mockRandom, mockGaParameter);
		actualChromosome.setEncodes(encodes);// 设置染色体编码
		actualChromosome.setCodes(codes);// 设置染色体编码方案

		actualChromosome.mutate(1);
		...
		Field field;
		try {
			field = actualChromosome.getClass().getDeclaredField("encodes");
			field.setAccessible(true);// 设置为可以测试的公有属性
			List<StringBuffer> actualEncodes = (ArrayList<StringBuffer>) field
					.get(actualChromosome);
			int actualLen = actualEncodes.size();
